...
let deezerSharedInstance = DeezerManager.sharedInstance
...
extension DeezerManager {
    ...
    private func save(token: String, expirationDate: Date, userId: String) {
        ...
        print_debug("ACCESS TOKEN    \(token)    USERID   \(userId)     expirationDate   \(expirationDate)")
...
extension DeezerManager: DeezerSessionDelegate {
    ...
    func deezerDidLogin() {
        ...
        guard let deezerConnect = deezerConnect else {return}
        
        var params = JSONDictionary()
        
        params["access_token"] = deezerConnect.accessToken
        
        let url = "https://api.deezer.com/user/me"

        WebServices.getDeezerUserDetaiAPI(url: url, params: params, webServiceSuccess: { (success, msg, json) in
            ...
            if status == "0" {
                ...
                let url = "https://www.deezer.com"
                NavigationManager.showPopUP(title: "Not a premium member", message: "For becoming premium member, use Deezer account for purchasing subscription." , url : url)
            }else{
                self.save(token: deezerConnect.accessToken, expirationDate: deezerConnect.expirationDate, userId: deezerConnect.userId)
